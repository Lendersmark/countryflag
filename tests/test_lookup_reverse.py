...
class TestReverseLookupEdgeCases:
    ...
    @pytest.mark.parametrize("emoji_flag,expected_country", [
        # Standard cases
        ("🇺🇸", "United States"),
        ("🇨🇦", "Canada"),
        ("🇩🇪", "Germany"),
    ...
    @pytest.mark.parametrize("emoji_flags,expected_pairs", [
...
class TestLookupHelperFunctions:
    ...
    @pytest.mark.parametrize("emoji_flag,expected", [
        ("🇺🇸", "🇺🇸"),  # Standard case
        ("🇬🇧", "🇬🇧"),  # Edge case
        ("  🇺🇸  ", "🇺🇸"),  # With whitespace
        ("", ""),  # Empty string
    ...
    @pytest.mark.parametrize("iso2_pattern,expected_codes", [
        ("US", ["US"]),
        ("^GB$|^UK$", ["GB", "UK"]),
        ("^GR$|^EL$", ["GR", "EL"]),
        ("not found", []),
    ...
    @pytest.mark.parametrize("emoji_flag,expected_iso", [
        ("🇺🇸", "US"),
        ("🇬🇧", "GB"),
        ("🇺🇰", "UK"),
        ("🇦🇨", "AC"),
    ...
    def test_get_iso_code_from_flag(
        self, 
        emoji_flag: str, 
        expected_iso: str
    ):
...
class TestEnhancedFlagMapping:
    ...
    def test_enhanced_mapping_size(self):
        ...
        for flag, country in mapping.items():
            ...
            assert len(flag) == 2  # Should be exactly 2 Unicode characters (regional indicators)
...
class TestIntegrationWithCountryFlag:
    ...
    def test_forward_and_reverse_consistency(self):
        ...
        for (flag, country), (original_country, original_flag) in zip(reverse_pairs, pairs):
