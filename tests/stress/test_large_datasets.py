...
class TestConcurrentOperations:
    ...
    def test_threaded_operations(self, medium_country_lists):
        ...
        for countries in medium_country_lists:
            thread = threading.Thread(target=lambda: results.append(convert_countries(countries)))
        ...
        sequential_time = sum(len(countries) * 0.01 for countries in medium_country_lists)
        assert execution_time < sequential_time, "Threaded execution was not faster than estimated sequential execution"
...
class TestCachePerformance:
    ...
    def test_memory_cache_with_large_dataset(self, large_country_list):
        ...
        assert time_hit < time_miss * 0.5, "Cache hit was not significantly faster than miss"
