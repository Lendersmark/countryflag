...
class TestTTLFunctionality:
    ...
    def ttl_cache(self, request, tmp_path):
        ...
        return TTLCache(base_cache, default_ttl=1, time_func=time_module.time)  # 1 second TTL
    ...
    def test_ttl_expiry_with_freezegun(self, tmp_path):
        ...
        memory_ttl_cache = TTLCache(MemoryCache(), default_ttl=1, time_func=time_module.time)
        disk_ttl_cache = TTLCache(DiskCache(str(tmp_path / "test_ttl_freezegun")), default_ttl=1, time_func=time_module.time)
        
        for cache_name, ttl_cache in [("memory", memory_ttl_cache), ("disk", disk_ttl_cache)]:
            # Set a value with 60 second TTL
            ttl_cache.set("frozen_key", "frozen_value", ttl=60)
            
            # Should be available immediately
            assert ttl_cache.get("frozen_key") == "frozen_value", f"{cache_name} cache: immediate get failed"
            assert ttl_cache.contains("frozen_key"), f"{cache_name} cache: immediate contains failed"
            
            # Move time forward by 30 seconds - should still be valid
            with freeze_time("2023-01-01 12:00:30"):
                assert ttl_cache.get("frozen_key") == "frozen_value", f"{cache_name} cache: 30s get failed"
                assert ttl_cache.contains("frozen_key"), f"{cache_name} cache: 30s contains failed"
            
            # Move time forward by 70 seconds - should be expired
            with freeze_time("2023-01-01 12:01:10"):
                assert ttl_cache.get("frozen_key") is None, f"{cache_name} cache: 70s get should be None"
                assert not ttl_cache.contains("frozen_key"), f"{cache_name} cache: 70s contains should be False"
...
class TestDiskCacheErrorHandling:
    ...
    def test_index_file_write_error(self, tmp_path):
        ...
        with patch.object(cache, '_save_index', side_effect=CacheError("Error saving cache index: Permission denied")):
...
class TestConcurrency:
    ...
    @pytest.mark.parametrize("cache_type,num_threads", [
        (MemoryCache, 5),
        (MemoryCache, 10),
    ...
    def test_concurrent_ttl_operations(self, tmp_path):
        ...
        base_cache = MemoryCache()
        ttl_cache = TTLCache(base_cache, default_ttl=3600)  # 1 hour TTL to avoid expiry issues
        ...
        assert set_get_ok_count == num_workers, f"Expected {num_workers} successful set/get, got {set_get_ok_count}. Results: {results}"
        assert contains_ok_count == num_workers, f"Expected {num_workers} successful contains, got {contains_ok_count}. Results: {results}"
        assert delete_ok_count == num_workers, f"Expected {num_workers} successful deletes, got {delete_ok_count}. Results: {results}"
        ...
        def concurrent_operations(thread_id):
            ...
            try:
                ...
                for i in range(50):
                    ...
                    if i % 10 == 0:
                        cache.set(f"new_key_{thread_id}_{i}", f"new_value_{thread_id}_{i}")
                        results.append("set")
...
class TestCacheIntegration:
    ...
    def test_disk_cache_persistence(self, tmp_path):
        ...
        assert cache2.get("complex_data") == {"list": [1, 2, 3], "nested": {"key": "value"}}
