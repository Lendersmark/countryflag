...
def run_benchmark(name: str, func: callable, args: Tuple = (), kwargs: Dict = None, 
                  iterations: int = 5, warmup: int = 1) -> Dict[str, Any]:
    """
...
def run_all_benchmarks(sizes: List[int] = None, iterations: int = 5, 
                       with_cache: bool = True) -> List[Dict[str, Any]]:
    """
...
def save_csv_report(results: List[Dict[str, Any]], filename: str) -> None:
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "Name", "Iterations", "Mean Time (s)", "Median Time (s)", 
            "Min Time (s)", "Max Time (s)", "Std Dev (s)"
        ])
...
def save_markdown_report(results: List[Dict[str, Any]], filename: str) -> None:
    ...
    with open(filename, 'w') as f:
        ...
        f.write("## Results\n\n")
        f.write("| Benchmark | Iterations | Mean (s) | Median (s) | Min (s) | Max (s) | Std Dev (s) |\n")
        f.write("|-----------|------------|----------|------------|---------|---------|-------------|\n")
        
        for result in results:
            f.write(f"| {result['name']} | {result['iterations']} | "
        ...
        f.write("| Dataset Size | No Cache (s) | Memory Cache Hit (s) | Improvement |\n")
        f.write("|--------------|--------------|----------------------|-------------|\n")
        ...
        for size in sorted(sizes):
            if size in no_cache_times and size in memory_cache_hit_times:
                ...
                f.write(f"| {size} | {no_cache_times[size]:.6f} | "
...
def generate_plots(results: List[Dict[str, Any]], output_dir: str) -> None:
    ...
    df['size'] = df['name'].apply(lambda x: int(x.split('_')[1]) if x.split('_')[0] == 'convert' else 0)
    df['type'] = df['name'].apply(lambda x: '_'.join(x.split('_')[2:]) if x.split('_')[0] == 'convert' else x.split('_')[0])
    ...
    cache_df = df[df['name'].str.contains('convert_') & 
...
def save_html_report(results: List[Dict[str, Any]], filename: str, 
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run benchmarks for the countryflag package")
    parser.add_argument("--output-dir", "-o", default="benchmark_results", 
                        help="Output directory for benchmark reports")
    parser.add_argument("--sizes", "-s", type=int, nargs="+", default=[5, 25, 100, 250, 500],
