...
from countryflag.utils.io import process_file_input, process_file_input_async, process_multiple_files
...
SAMPLE_COUNTRIES = [
    ...
    "Portugal", "Ireland", "Iceland", "New Zealand", "Singapore"
]
...
def process_in_chunks(countries: List[str], chunk_size: int = 500) -> List[Tuple[str, str]]:
    """
...
def process_in_parallel(countries: List[str], num_workers: int = 4, chunk_size: int = 500) -> List[Tuple[str, str]]:
    """
...
async def process_async(countries: List[str], chunk_size: int = 500) -> List[Tuple[str, str]]:
    """
...
def process_with_cache(countries: List[str], cache_type: str = "memory") -> List[Tuple[str, str]]:
    """
...
def benchmark_methods():
    ...
    print(f"Processed {len(file_results)} countries from file in {file_time:.2f} seconds")
    ...
    print(f"Processed {len(async_file_results)} countries from file in {async_file_time:.2f} seconds")
    ...
    print(f"Processed {len(multiple_files_results)} countries from multiple files in {multiple_files_time:.2f} seconds")
    ...
    print(f"Processed {len(memory_cache_results)} countries with memory cache (miss) in {memory_cache_miss_time:.2f} seconds")
    
    # Second run (cache hit)
    start_time = time.time()
    memory_cache_results = process_with_cache(large_dataset, "memory")
    memory_cache_hit_time = time.time() - start_time
    print(f"Processed {len(memory_cache_results)} countries with memory cache (hit) in {memory_cache_hit_time:.2f} seconds")
    print(f"Memory cache speedup: {memory_cache_miss_time / memory_cache_hit_time:.2f}x")
    ...
    print(f"Processed {len(disk_cache_results)} countries with disk cache (miss) in {disk_cache_miss_time:.2f} seconds")
    
    # Second run (cache hit)
    start_time = time.time()
    disk_cache_results = process_with_cache(large_dataset, "disk")
    disk_cache_hit_time = time.time() - start_time
    print(f"Processed {len(disk_cache_results)} countries with disk cache (hit) in {disk_cache_hit_time:.2f} seconds")
    print(f"Disk cache speedup: {disk_cache_miss_time / disk_cache_hit_time:.2f}x")
    
    # Summary
    print("\nSummary:")
    print(f"Chunking: {chunk_time:.2f} seconds")
    print(f"Parallel: {parallel_time:.2f} seconds (Speedup: {chunk_time / parallel_time:.2f}x)")
    print(f"Async: {async_time:.2f} seconds (Speedup: {chunk_time / async_time:.2f}x)")
    print(f"Memory Cache (hit): {memory_cache_hit_time:.2f} seconds (Speedup: {chunk_time / memory_cache_hit_time:.2f}x)")
    print(f"Disk Cache (hit): {disk_cache_hit_time:.2f} seconds (Speedup: {chunk_time / disk_cache_hit_time:.2f}x)")
