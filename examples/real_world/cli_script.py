...
def process_countries(
    ...
    try:
        ...
        flags, pairs = cf.get_flag(countries, separator=separator, fuzzy_matching=fuzzy_matching)
        ...
        if fuzzy_matching:
            ...
            return process_countries(countries, output_format, separator, True, cache_type, cache_dir)
        else:
            raise
...
def list_countries(output_format: str = "text") -> str:
    ...
    if output_format == "json":
        ...
        return "\n".join([f"{country['name']} ({country['iso2']})" for country in countries])
...
def main():
    ...
    input_group.add_argument(
        "--list-regions",
        action="store_true",
        help="List all supported regions"
    )
    input_group.add_argument(
        "--list-countries",
        action="store_true",
    ...
    cache_group.add_argument(
        "--cache",
        choices=["memory", "disk"],
        help="Enable caching (memory or disk)"
    )
    cache_group.add_argument(
        "--cache-dir",
        metavar="DIR",
