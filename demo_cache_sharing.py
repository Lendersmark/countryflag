...
def main():
    ...
    print(f"   Global cache hits after cf1 call: {CountryFlag._global_cache.get_hits()}")
    print()
    
    print("3. Using cf2 to get flag for the same country (Germany):")
    flags2, pairs2 = cf2.get_flag(["Germany"])
    print(f"   Result: {flags2}")
    print(f"   Global cache hits after cf2 call: {CountryFlag._global_cache.get_hits()}")
    ...
    print(f"   cf3 cache is different from global: {cf3._cache is not CountryFlag._global_cache}")
    ...
    print(f"   Global cache hits before deleting cf1, cf2: {CountryFlag._global_cache.get_hits()}")
    ...
    print(f"   Global cache hits (should be same): {CountryFlag._global_cache.get_hits()}")
    
    # Request same country again - should hit cache
    flags4, pairs4 = cf4.get_flag(["Germany"])
    print(f"   cf4 result for Germany: {flags4}")
    print(f"   Global cache hits after cf4 call: {CountryFlag._global_cache.get_hits()}")
    print()
